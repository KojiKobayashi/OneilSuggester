...
| ブランチ | URL | 更新タイミング |
|---|---|---|
| `main` | https://kojikobayashi.github.io/OneilSuggester/ | 毎朝自動実行（スケジュール、実行前に銘柄リストも更新） |
| `develop` | https://kojikobayashi.github.io/OneilSuggester/develop/ | `develop` ブランチへのプッシュ時（実行前に銘柄リストも更新） |
...
- 各銘柄をスコアリングし、上位銘柄を `docs/data/YYYY-MM-DD.json` に出力
- GitHub Actions で毎朝自動実行（`main`）、および `develop` ブランチ更新時に自動実行
- バッチ実行前に `batch/update_stock_list.py` を走らせ、JPX の最新銘柄リストを取り込む
...
```bash
# 銘柄リストの更新
uv run python batch/update_stock_list.py
...
| `docs/data/index.json` | 利用可能な結果日付の一覧 |
| `docs/data/YYYY-MM-DD.json` | バッチ処理の出力（スクリーニング結果） |
...
│   └── data/YYYY-MM-DD.json     # スクリーニング結果（自動更新）
├── data/
